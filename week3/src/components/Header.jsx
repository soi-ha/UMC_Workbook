import { Link } from 'react-router-dom';
export default function Header({ color, onChange }) {
	return (
		<section className="header">
			<div className="customer">
				<span className="cus-content sign-up">회원가입</span>
				{/* <span className="cus-content sign-in">로그인</span> */}
				<Link className="cus-content sign-in" style={{ textDecoration: 'none' }} to={'/login'}>
					로그인
				</Link>
				<span className="cus-content support">고객센터</span>
			</div>
			<div className="title">
				<div className="title-left">
					<img
						src="data:image/svg+xml;base64,"
						alt="마켓컬리 로고"
					/>
					<Link
						className={`left-content market ${color === 'market' ? 'activeColor' : ''}`}
						onClick={() => onChange('market')}
						style={{ textDecoration: 'none' }}
						to={'/'}
					>
						마켓컬리
					</Link>
					<Link
						className={`left-content beauty ${color === 'beauty' ? 'activeColor' : ''}`}
						onClick={() => onChange('beauty')}
						style={{ textDecoration: 'none' }}
						to={'/beauty'}
					>
						뷰티컬리
					</Link>
					{/* <span
						className={`left-content market ${color === 'market' ? 'activeColor' : ''}`}
						onClick={() => onChange('market')}
					>
						마켓컬리
					</span>
					<span
						className={`left-content beauty ${color === 'beauty' ? 'activeColor' : ''}`}
						onClick={() => onChange('beauty')}
					>
						뷰티컬리
					</span> */}
				</div>
				<div className="title-middle">
					<div className="search">
						<input className="search-box" type="text" required placeholder="검색어를 입력해주세요" />
						<button className="submit"></button>
					</div>
				</div>
				<div className="title-right">
					<div className="right-icon location"></div>
					<div className="right-icon heart"></div>
					<div className="right-icon shopping"></div>
				</div>
			</div>

			<div className="nav">
				<div className="category">
					<div className="cate-content cate-icon"></div>
					<span className="cate-content cate-text">카테고리</span>
				</div>
				<ul className="tab">
					<li className="tab-content">신상품</li>
					<li className="tab-content">베스트</li>
					<li className="tab-content">알뜰쇼핑</li>
					<li className="tab-content">특가/혜택</li>
				</ul>
				<div className="guide">
					<span className="guide-style">샛별・택배</span> 배송안내
				</div>
			</div>
		</section>
	);
}
